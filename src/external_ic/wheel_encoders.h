...
#define RIGHT_ENC_IX 0
#define LEFT_ENC_IX 1
...
namespace SVEA
{
    ...
    private:
        // Ros stuff
        SVEA::NodeHandle &nh;
        ros::Publisher wheel_encoders_pub;
        ...
        static uint32_t ENC_TICK_TIME[2][2]; // Time of last right and left tick event

        //  Switch to the next pwm duration buffer and return the previous buffer index
    ...
    public:
        WheelEncoders(SVEA::NodeHandle &nh) : nh(nh),
                                              wheel_encoders_pub("lli/encoder", &wheel_encoders_msg)
        {
            ENC_BUFFER_IX = 0;
            ...
            attachInterrupt(digitalPinToInterrupt(ENCODER_PIN_R2), rightEncoderIsr, CHANGE);
            attachInterrupt(digitalPinToInterrupt(ENCODER_PIN_L2), leftEncoderIsr, CHANGE);
        ...
        void update()
        {
            ...
            if (processEncoderTicks(reading))
            {
                wheel_encoders_msg.right_ticks = reading.right_ticks;
                wheel_encoders_msg.left_ticks = reading.left_ticks;
                wheel_encoders_msg.right_time_delta = reading.right_time_delta;
                wheel_encoders_msg.left_time_delta = reading.left_time_delta;
